...
Net::Request AddProductModel::FormRequest(const Product<QString>& product)
{
	Net::Request request;
	...
	request.json_payload = m_formatter.Format(product);
	return request;
}
...
ProductId AddProductModel::ParseResponse(const Net::Response& response)
{
	...
	product_id = m_parser.Parse(response.json_payload);
