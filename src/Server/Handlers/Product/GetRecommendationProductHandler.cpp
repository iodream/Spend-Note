...
Net::Response GetRecommendationProductHandler::AuthHandle(const Net::Request& request)
{
	...
	if (!category.has_value()) {
			throw InternalError(std::string("No product category with id:") + std::to_string(db_product.value().category_id));
	}
	auto product = ToNetProduct(db_product.value(), category.value());
